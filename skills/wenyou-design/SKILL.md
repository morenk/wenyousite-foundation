...
6. 用真实中文内容构建并截图自审。检查层级、字形、对比度、溢出、焦点、空/错/加载态和减少动态效果；集合页面还要按 `experiences.collections` 检查列表容器与列表项占满分配列、短内容不收缩；图片任务还要检查裁切、比例预留、失败/受限状态、替代文本和完整查看出口；删除不承载信息的装饰。
