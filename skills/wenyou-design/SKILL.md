...
任务涉及主题、黑夜模式或系统外观时，还要读取顶层 `themes`、两种 palette、两组等级色以及目标平台的浮层契约；不得把亮色色值机械反转或在客户端建立第二套色板。
...
不得从本 Skill 记忆或重写色值、排版数值、能力清单或图标映射。目标仓库的锁定 foundation 版本与当前仓库不一致时，以目标仓库锁定版本为准；需要新语义时先更新中央契约并发布版本。图标任务还必须读取 `docs/icons.md` 与 `experiences.icons`。
...
3. 对照共享基础自审：亮色白纸感或黑夜温暖墨紫阅读面、柔粉克制、信息不丢失、无虚假装饰、文案使用真实功能名称。
4. 对照平台 profile 落地：Web 保留桌面密度和键鼠效率；Flutter 保留单列、48dp 触控、安全区、文字缩放和系统返回。不要用像素相同替代体验一致。
5. 只消费中央 package 暴露的 Token、排版角色和体验契约；字体由平台系统提供。业务代码不得写近似色值、复制能力矩阵或建立第二套组件语义。
6. 用真实中文内容构建并截图自审。主题任务同时覆盖亮色、黑夜、跟随系统和刷新恢复，并检查首屏闪烁、浏览器控件与外部图片；其余检查包括语义排版、层级、字形、对比度、溢出、焦点、空/错/加载态、pending 防重和减少动态效果；集合页面还要按 `experiences.collections` 检查列表容器与列表项占满分配列、短内容不收缩；图片任务还要检查裁切、比例预留、失败/受限状态、替代文本和完整查看出口；导航与反馈文案必须使用中央标签和稳定动词；删除不承载信息的装饰。
