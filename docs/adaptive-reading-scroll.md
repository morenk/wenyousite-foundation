...
- 展开态在滑块下方使用 24×64dp、0.92 透明度的 surface 局部底衬；底衬与展开进度同步渐显并保持同一纵向中心，细态不显示。它只保护图片等极端后景上的非文字对比，不是整轨背景。
...
- 无可滚动内容不显示。普通慢读沿用细条：正文停稳后保持 1500ms，再用 180ms 淡出。
...
- 展开使用 180ms easeOutCubic，无回弹；再次快滑从动画当前值继续展开。reduced motion 直接切换形态。
...
- 普通 pointer-up 应用最后输入并停止末端跟随，正文停稳后保持展开 1000ms，再以 240ms easeInOutCubic 收细；细态保持 600ms，最后以 180ms 淡出。pointer-up 不等同取消或 controller dispose。
