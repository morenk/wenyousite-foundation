...
- v7.2.1 仅收紧新 adaptive profile：细态紧贴页面右侧安全边缘、展开态才应用既有 `edgeGap`，展开停稳保持改为 1000ms；展开白色局部底衬移除，backing 兼容字段保留且 opacity 为 0，平台可见焦点反馈继续保留。显现／消失与中途反向补充连续、按剩余距离缩时的机器语义；100ms 快滑窗口内阈值调整为 40ms／40dp／550dp/s／0.8，三条件与输入排除语义不变。慢读 1500ms 与旧阅读快翻 profile/API 不变。Mobile 在正式 Tag 前继续锁定 `v7.2.0`。
