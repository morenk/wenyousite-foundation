...
- 接口和安全策略以 [Backend 5.25.0-dev.20260921.1 的账号与安全边界](https://github.com/morenk/wenyousite-backend/blob/1e8d19c1492b62a2ff191647bf0f10ead1b0fdfd/docs/modules/admin.md#账号与安全边界) 为准。登录验证省略或不启用记住设备时保持原短会话规则；既有数据库会话迁移为未启用，不延长原期限。固定七天模式不滚动续期，也不改变单会话、角色和账号状态校验、CSRF 或独立的高风险确认边界。
