...
细态视觉以页面右侧安全边缘为基准，不因 48dp 命中区居中而内缩；展开视觉才向左应用既有 edge gap，命中区继续避开系统手势区。adaptive 展开态不显示白色局部底衬，兼容字段保留且 opacity 为 0；键盘与 TalkBack 的平台可见焦点反馈不受影响。正文停稳后的展开保持为 1000ms，慢读细条仍保持 1500ms 后淡出；显现／消失及中途反向使用连续、按剩余距离缩时的过渡。快滑识别在保留 100ms 窗口、三条件同时满足及既有排除规则的前提下，采用 40ms／40dp／550dp/s／0.8 阈值。根包、Flutter 包与契约版本为 `7.2.1`，`schemaVersion` 保持 `3`；消费者仅在正式 `v7.2.1` Tag 发布后升级。
